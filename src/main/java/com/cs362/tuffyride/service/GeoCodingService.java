...
import com.cs362.tuffyride.exception.GeoCodingException;
import com.cs362.tuffyride.exception.InvalidRideAddressException;
    ...
    public Location getLatLng(Long id, String address) throws GeoCodingException {
        try {
            ...
            if (result.partialMatch) {
                throw new InvalidRideAddressException("Failed to find ride address");
            }
            return new Location(id, new GeoPoint(result.geometry.location.lat, result.geometry.location.lng));
        } catch (IOException | ApiException | InterruptedException e) {
            e.printStackTrace();
            throw new GeoCodingException("Failed to encode ride address");
        }
    }
}
